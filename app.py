...
if pagina == "Produção":
    ...
    st.download_button(
        ...
        data=buffer,
        file_name=f"consolidado_{mes_sel}_{anos_sel}.xlsx",
...
# ------------------------------------------------------
    ...
    def gerar_tabela_ranking(ranking, grupo, mes, ano, tipo_ranking):
        ...
        buffer = io.BytesIO()
        df_tab.to_excel(buffer, index=False, engine="openpyxl")
        buffer.seek(0)

        st.download_button(
            label="Baixar tabela em Excel",
            data=buffer,
            file_name=f"ranking_{grupo}_{mes}_{ano}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
